...
int[] points = { 0, 0, 0, 0, 0, 0 }; // инициализируем массив, в который будем считывать координаты
...
void TakePoints(int[] array)

{ // Метод забирающий координаты точек из консоли
    Console.Write("Введите координаты точек:"); // выводим сообщение пользователю
...
string dataBetween(char leftBorder, char rightBorder, string inData, ref int startPosition)
{
    ...
    while (numStart <= inData.Length - 1 && inData[numStart] != leftBorder)
    { // ищем символ левой границы и его номер, выходим когда нашли, либо дошли до конца
        numStart++;
    }

    int numFinish = numStart + 1; // номер символа правой границы, задаем его как следующий за левой границей

    while (numFinish <= inData.Length - 1 && inData[numFinish] != rightBorder)
    { // ищем символ правой границы и его номер, выходим когда нашли, либо дошли до конца
        numFinish++;
    }
    ...
    while (i < numFinish)
    { // Собираем в результат символы между граничными
